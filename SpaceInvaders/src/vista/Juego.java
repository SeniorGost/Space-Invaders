...
public class Juego extends JPanel implements Observer {
	...
	// creamos una matriz de JLabels para no perder las JLabels y asi poder
	// modificarlas despues (De momento va a tener un tamaño fijo de 60 filas *100
	// columnas)
	// private JLabel[][] pixel= new JLabel[60][100];

	// forma alt para matriz de cualquier tamaño (commentada ahora para hacer las
	// pruebas)
	private int filas;
	...
	public void update(Observable o, Object arg) {
		// para hacer lo de matriz de cualquier tamaño voy a necesitar que me envieis el
		// tamaño por aqui
		if (o instanceof Modelo) {

			// Aqui he cambiado el orden para revisar primero si se ha dado un tick y
			// después si se ha cambiado a esta pantalla (Esto es porque como ocurrirá más
			// frecuentemente que se de un tick a que se cambie a esta pantalla es
			// ligeramente más eficiente)
			// Por cierto, esta comprobación de aqui abajo funciona porque desde modelo
			// enviamos solo notifyobservers(-1), luego por lo del 'autoboxing' que hace el
			// metodo, java asume que el -1 es integer, luego no hay instancia de int (Lo
			// mismo ocurre si envias un objeto int tal que: int a = -1; notifyObservers(a);
			// por defecto java lo combierte a integer al enviarlo)
			if (arg instanceof Integer) {
				cleanClean();
				pintarPantalla();
			} else {
				// Si llegamos a este else sabemos que nos esta llegando instanceof int[],
				// porque no puede ser de otra forma
				int[] datos = (int[]) arg;

				if (datos[0] == Modelo.NOTIFY_WINDOW_CHANGE) {
					if (datos[1] == 2) {
						// esto le dice que si se pasa a la pantalla 1, que primero defina la matriz con
						// las dimensiones que me pasa modelo
						definirMatriz(datos[2], datos[3]);
					}
					// Esto es pq si no al empezar la partida escribes directamente encima de la
					// matriz otra vez y se notan un segundo los restos de la otra
	...
	// Este es el tema, cada vez que se notifica a los observers, se notifica a
	// todas las pantallas a la vez entonces se tiene que poner una sentencia como
	// esta, donde se distinga el numero de pantalla en que estamos
	private void cambiarPantalla(int pValor) {
		if (pValor == 2)
			MyFrame.getMyFrame().cambiarVentana(2);
	}
	...
	private void definirMatriz(int c, int f) {
		...
		// si ya existen las labels (ya se ha jugado una partida) Hay que borrar las
		// labels de la partida anterior
		if (pixel != null) {
			for (int fila = 0; fila < this.filas; fila++) {
				for (int columna = 0; columna < this.columnas; columna++) {
					remove(pixel[fila][columna]);
				}
			}
		}
...
//			}
		...
		for (int fila = 0; fila < this.filas; fila++) {
			for (int columna = 0; columna < this.columnas; columna++) {
				// Todas las Jlabels se añaden a la matriz para no perderlas y luego se meten en
				// la vista
				pixel[fila][columna] = new JLabel();
				add(pixel[fila][columna]);
	...
	protected void paintComponent(java.awt.Graphics componente) {
		...
		componente.drawImage(new ImageIcon(Menu.class.getResource("/spritesEpicos/background.jpg")).getImage(), 0, 0,
